...
for video in "$INPUT_DIR"/*.{mp4,avi,mkv,mov,yuv,264,h264}; do
    ...
    # Archivo de salida
    output_file="$OUTPUT_DIR/${name}_BR.webm"
    ...
    if [[ "$filename" == *.yuv ]]; then
        ...
        ffmpeg -f rawvideo -pixel_format yuv420p -video_size $resolution -framerate 24 \
               -i "$video" -c:v libvpx -b:v $BITRATE -y "$output_file" &
    else
        ffmpeg -i "$video" -c:v libvpx -b:v $BITRATE -c:a libvorbis -y "$output_file" &
    fi

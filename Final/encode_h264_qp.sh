...
for video in "$INPUT_DIR"/*.{mp4,avi,mkv,mov,yuv,264,h264}; do
    ...
    # Archivo de salida
    output_file="$OUTPUT_DIR/${name}_QP.mp4"
    ...
    if [[ "$filename" == *.yuv ]]; then
        ...
        ffmpeg -f rawvideo -pixel_format yuv420p -video_size $resolution -framerate 24 \
               -i "$video" -c:v libx264 -qp $QP -y "$output_file" &
    else
        ffmpeg -i "$video" -c:v libx264 -qp $QP -c:a copy -y "$output_file" &
    fi

...
LOGS_DIR="cpu_logs/h264/BR"
mkdir -p "$LOGS_DIR"

echo "==================================="
echo "Codificación H264"
...
for video in "$INPUT_DIR"/*.{mp4,avi,mkv,mov,yuv,264,h264}; do
    ...
    # Archivo de salida
    output_file="$OUTPUT_DIR/${name}_BR.mp4"
    
    # Archivo CSV para logs de CPU
    csv_file="$LOGS_DIR/${name}_BR_cpu.csv"
    ...
    if [[ "$filename" == *.yuv ]]; then
        ...
        ffmpeg -f rawvideo -pixel_format yuv420p -video_size $resolution -framerate 24 \
               -i "$video" -c:v libx264 -b:v $BITRATE -y "$output_file" &
    else
        ffmpeg -i "$video" -c:v libx264 -b:v $BITRATE -c:a copy -y "$output_file" &
    fi
